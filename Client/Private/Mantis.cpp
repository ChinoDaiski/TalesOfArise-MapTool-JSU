...
HRESULT CMantis::NativeConstruct(void * pArg)
{
	...
	_vector vPos = XMVectorSet(20.f, 0.f, 0.f, 1.f);
...
void CMantis::Tick(_double TimeDelta)
{
	...
	_vector vMantisPos = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	...
	if (!m_bHit && m_bStartScene && m_bBattle)		// 플레이어가 공격을 하지 않는다면 동작 개시
	{
		m_TimeDelta += TimeDelta;
...
void CMantis::AttackPattern5()
{
	...
	if (m_iCurrentAnimationIndex == SHOCK_WAVE)
	{
		...
		{
			m_bOnAttackCollider = true;
			m_pAttackSphereCom->Set(m_pTransformCom->Get_State(CTransform::STATE_POSITION) + m_pTransformCom->Get_State(CTransform::STATE_LOOK)*1.3f + m_pTransformCom->Get_State(CTransform::STATE_RIGHT) + XMVectorSet(0.f, 2.f, 0.f, 0.f), 2.f);
		}
	}
