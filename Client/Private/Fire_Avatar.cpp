...
void CFire_Avatar::RockFalling(_double TimeDelta)
{
	...
	if ((m_iCurrentAnimationIndex == ATTACK_ROCK_THROW) && !m_bCreateRock)
	{
		if (m_pModelCom->Get_CurAnimation()->Get_PelvisChannel()->Get_CurrentKeyFrameIndex() >= 220 && m_pModelCom->Get_CurAnimation()->Get_PelvisChannel()->Get_CurrentKeyFrameIndex() <= 227)
		{
			...
			SocketDesc.pModelCom = (CModel*)pGameInstance->Get_Component(LEVEL_TUTORIAL, TEXT("Layer_FireAvatar"), TEXT("Com_Model"));
			SocketDesc.pBoneName = "middle_01_R";

			if (nullptr == (pGameInstance->Add_GameObjectToLayer(LEVEL_TUTORIAL, TEXT("Layer_FireAvatar"), TEXT("Prototype_GameObject_FireAvatar_Rock"), &SocketDesc)))
				return;
