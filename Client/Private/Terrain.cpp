...
HRESULT CTerrain::SetUp_NaviComponent(const TCHAR * pNaviDataFilePath)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

	if (FAILED(pGameInstance->Add_Prototype(LEVEL_MapTool, TEXT("Prototype_Component_Navigation"),
		CNavigation::Create(m_pDevice, m_pDeviceContext))))
		return E_FAIL;
	...
	/* For.Com_Renderer */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Navi"), LEVEL_MapTool, TEXT("Prototype_Component_Navigation"), (CComponent**)&m_pNaviCom, &NaviDesc)))
		return E_FAIL;
