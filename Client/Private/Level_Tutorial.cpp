...
HRESULT CLevel_Tutorial::NativeConstruct()
{
	...
	CImGUI_Manager::GetInstance()->OnOffImgui();
...
HRESULT CLevel_Tutorial::Render()
{
	...
	if (pGameInstance->Key_Down(DIK_1)) {
		//pGameInstance->Open_Level(LEVEL_LOADING, CLevel_Loading::Create(m_pDevice, m_pDeviceContext, LEVEL_BALSEPH));	// -> 인게임 이동

		// 기존의 오브젝트 삭제
		CLayer* pLayer = pGameInstance->Find_Layer(LEVEL_TUTORIAL, LAYER_MAPTOOL_PlacedObjects);
		auto& ObjList = pLayer->Get_ObjectList();

		for (auto& Object : ObjList) {
			Object->Set_Dead(true);
		}

		if (FAILED(Ready_Map("../Bin/Resources/Dat/Stage01/", "MeshList.dat")))
			return E_FAIL;

		CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_Tutorial"));

		CTransform* pAlphen_TransformCom = dynamic_cast<CTransform*>(m_pPlayerAlphen->Get_Component(TEXT("Com_Transform")));
		pAlphen_TransformCom->Move(119.f, 0.f, 304.f);	// Tutorial Map 시작 위치
		pAlphen_TransformCom->Move(119.f, pAlphen_TransformCom->Get_Height(pNavigation), 304.f);
		pNavigation->Find_My_Cell(XMVectorSet(119.f, 0.f, 304.f, 1.f));
	}
	...
	if (pGameInstance->Key_Down(DIK_2)) {
		...
		CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_BalsephEnterance"));

		CTransform* pAlphen_TransformCom = dynamic_cast<CTransform*>(m_pPlayerAlphen->Get_Component(TEXT("Com_Transform")));
		pAlphen_TransformCom->Move(35.91f, 3.84f, 0.37f);	// Tutorial Map 시작 위치
		pAlphen_TransformCom->Move(35.91f, pAlphen_TransformCom->Get_Height(pNavigation), -242.93f);
		pNavigation->Find_My_Cell(XMVectorSet(35.91f, 3.84f, 0.37f, 1.f));
	}
	...
	if (pGameInstance->Key_Down(DIK_3)) {
		...
		CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_Balseph"));

		CTransform* pAlphen_TransformCom = dynamic_cast<CTransform*>(m_pPlayerAlphen->Get_Component(TEXT("Com_Transform")));
		pAlphen_TransformCom->Move(-75.31f, 19.25f, 0.02f);	// Tutorial Map 시작 위치
		pAlphen_TransformCom->Move(-75.31f, pAlphen_TransformCom->Get_Height(pNavigation), 0.02f);
		pNavigation->Find_My_Cell(XMVectorSet(-75.31f, 19.25f, 0.02f, 1.f));
	}
	...
	if (pGameInstance->Key_Down(DIK_4)) {
		...
		if (FAILED(Ready_Map("../Bin/Resources/Dat/Stage04/", "MeshList.dat")))
			return E_FAIL;

		CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_FireAvatar"));

		CTransform* pAlphen_TransformCom = dynamic_cast<CTransform*>(m_pPlayerAlphen->Get_Component(TEXT("Com_Transform")));
		pAlphen_TransformCom->Move(-91.22f, 2.42f, 0.32f);	// Tutorial Map 시작 위치
		pAlphen_TransformCom->Move(91.22f, pAlphen_TransformCom->Get_Height(pNavigation), 0.32f);
		pNavigation->Find_My_Cell(XMVectorSet(-91.22f, 2.42f, 0.32f, 1.f));
	}
	...
	if (pGameInstance->Key_Down(DIK_5)) {
		...
		if (FAILED(Ready_Map("../Bin/Resources/Dat/Stage08/", "MeshList.dat")))
			return E_FAIL;

		CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_GrandYork_Battle_02"));

		CTransform* pAlphen_TransformCom = dynamic_cast<CTransform*>(m_pPlayerAlphen->Get_Component(TEXT("Com_Transform")));
		pAlphen_TransformCom->Move(0.f, 0.f, 0.f);	// Tutorial Map 시작 위치
	...
	if (pGameInstance->Key_Down(DIK_6)) {
		...
		if (FAILED(Ready_Map("../Bin/Resources/Dat/Stage09/", "MeshList.dat")))
			return E_FAIL;

		CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_GrandYork_Battle_04"));
...
HRESULT CLevel_Tutorial::Ready_Layer_Player(const _tchar * pLayerTag)
{
	...
	//// 튜토리얼 맵의 Navigation Mesh를 Alphen이 타게 함.
	//CNavigation* pNavigation = m_pPlayerAlphen->SetUp_Navigation(TEXT("Prototype_Component_Navigation_Map_BalsephEnterance"));
	...
	//pAlphen_TransformCom->Move(352.51f, -18.42f, -242.93f);	// Tutorial Map 전투 시작 위치
	//pAlphen_TransformCom->Move(352.51f, pAlphen_TransformCom->Get_Height(pNavigation), -242.93f);
	//pNavigation->Find_My_Cell(XMVectorSet(352.51f, -18.42f, -242.93f, 1.f));
	...
	CImGUI_Manager::GetInstance()->Set_Client_Player(m_pPlayerAlphen);
...
HRESULT CLevel_Tutorial::Ready_Layer_HP(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	//초록이
...
HRESULT CLevel_Tutorial::Ready_Layer_HPRed(const _tchar * pLayerTag)
{
	CGameInstance*		pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	//빨강이
...
HRESULT CLevel_Tutorial::Ready_Map(const char * pModelFilePath, const char * pModelFileName)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
