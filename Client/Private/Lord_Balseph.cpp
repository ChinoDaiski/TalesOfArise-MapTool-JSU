...
void CLord_Balseph::Tick(_double TimeDelta)
{
	...
	if (m_iCurrentAnimationIndex == m_iNextAnimationIndex)
	{
		...
		m_pModelFaceCom->Update(TimeDelta*m_dAnimSpeed, m_bCutAnimation);
	...
	else
	{
		...
		m_pModelFaceCom->Update_NextAnim(TimeDelta*m_dAnimSpeed, m_iDuration, m_iNextAnimationIndex, &m_iCurrentAnimationIndex);
	...
	if (m_bStartScene && m_pTransformCom->Distance(pPos, 20.f))
	{
		StartScene();
	}

	if (!m_bHit && m_bStartScene)
	{
