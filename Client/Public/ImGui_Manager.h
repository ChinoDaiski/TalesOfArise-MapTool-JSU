...
private:
	...
	_bool Check_SameCell(_float3* pPoints);	// 인자로 점 3개를 받아서 m_vCell에 있는 셀 정보중 같은 점의 정보로 만들어진 셀이 있는지 확인하여 있다면 true, 아니라면 false를 반환하는 함수
	_bool Check_Cell_Point(_float3& pPoint, CCell* pCell);	// 인자로 점 1개와 셀 정보를 받아서, 인자로 받은 점이 인자로받은 셀의 점중 하나라면 true, 아니라면 false를 반환하는 함수
