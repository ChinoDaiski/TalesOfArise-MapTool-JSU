...
HRESULT CRenderer::NativeConstruct_Prototype()
{
	...
	m_pShader = CShader::Create(m_pDevice, m_pDeviceContext, TEXT("../Bin/ShaderFiles/Shader_Deferred.hlsl"), VTXTEX_DECLARATION::Elements, VTXTEX_DECLARATION::iNumElements, CShader::FX);
	...
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_NonBlur"), Viewport.Width - 400.f, 0.f, 200.f, 200.f)))
	{
		MSG_BOX(L"Failed To CRenderer : NativeConstruct_Prototype : m_pTarget_Manager->Ready_DebugDesc(Target_NonBlur)");
		return E_FAIL;
	}

	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Blur"), Viewport.Width - 400.f, 200.f, 200.f, 200.f)))
	{
		MSG_BOX(L"Failed To CRenderer : NativeConstruct_Prototype : m_pTarget_Manager->Ready_DebugDesc(Target_Blur)");
		return E_FAIL;
	}

	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_ObjectGlow"), Viewport.Width - 400.f, 400.f, 200.f, 200.f)))
	{
		MSG_BOX(L"Failed To CRenderer : NativeConstruct_Prototype : m_pTarget_Manager->Ready_DebugDesc(Target_Blur)");
		return E_FAIL;
	}
...
HRESULT CRenderer::Render_Deferred()
{
	...
	//m_pShader->Begin(4);
	m_pShader->Begin(3);		// 검사를 위해서 교체, 실제론 4로 바꾸어 실행할것
